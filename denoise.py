...
def get_skew_angle(image):
    # Tính góc nghiêng văn bản sử dụng Hough Transform
...
def rotate_image(image, angle):
    # Xoay ảnh để cân chỉnh thẳng hàng
    (h, w) = image.shape[:2]
    center = (w // 2, h // 2)
...
def smart_noise_filter(binary_img, min_area=5):
    # Lọc bỏ các vùng nhiễu nhỏ hơn min_area (giữ lại dấu nặng, xóa bụi)
...
def calculate_accuracy(ground_truth, ocr_result):
    # Tính độ chính xác (Accuracy) và tỷ lệ lỗi (CER/WER)
...
def process_pipeline(
    ...
    # Pipeline chính: Deskew -> Denoise -> Threshold -> Smart Filter -> OCR -> Eval
    if image is None:
        return None, "", ""
    ...
    # 4. Smart Noise Filter (Quan trọng)
        ...
        with gr.Column(scale=2):
            with gr.Tabs():
                with gr.TabItem("Ảnh nhị phân"):
                    output_img = gr.Image(label="Result Image", type="numpy")
